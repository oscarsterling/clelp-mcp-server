...
const searchCooldowns: Map<string, Map<string, number>> = new Map(); // agentKey -> skillId -> timestamp
...
function trackSearch(apiKey: string, skillId: string) {
...
function checkCooldown(apiKey: string, skillId: string): { allowed: boolean; waitMinutes?: number } {
  const searches = searchCooldowns.get(apiKey);
  if (!searches) return { allowed: true };
  
  const searchTime = searches.get(skillId);
  if (!searchTime) return { allowed: true };
...
async function handleSearch(args: { query: string; category?: string; type?: string; limit?: number }) {
  ...
  if (CLELP_API_KEY && Array.isArray(skills)) {
    skills.forEach((skill: any) => trackSearch(CLELP_API_KEY, skill.id));
  }
  
  // Format response
  const results = skills.map((skill: any) => ({
    id: skill.id,
    name: skill.name,
    slug: skill.slug,
